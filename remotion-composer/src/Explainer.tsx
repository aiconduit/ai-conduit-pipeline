...
interface Cut {
  ...
  vignette?: boolean;
  // Cinematic title / Stat counter / Subscribe CTA props
  stats?: Array<{label: string; value: number; suffix?: string; color?: string}>;
  handle?: string;
  message?: string;
  ctaText?: string;
  lightingFrom?: string;
...
const SceneRenderer: React.FC<{ cut: Cut; theme: ThemeConfig }> = ({ cut, theme }) => {
  ...
  // --- Pexels B-roll scene ---
  if (cut.type === "pexels_scene") {
    return (
      <PexelsScene
        videoUrl={cut.source || undefined}
        text={cut.text || undefined}
        textPosition={(cut.textPosition as any) || "bottom"}
        accentColor={accent}
        overlayOpacity={cut.overlayOpacity as number | undefined}
      />
    );
  }

  // --- Cinematic title scene ---
  if (cut.type === "cinematic_title") {
    return (
      <CinematicTitle
        title={cut.text || "Today AI Trend"}
        subtitle={cut.subtitle || "GitHub Trending"}
        accentColor={accent}
        backgroundColor={bgColor || theme.backgroundColor}
      />
    );
  ...
  // --- Stat counter scene ---
  if (cut.type === "stat_counter" && cut.stats) {
    return (
      <StatCounter
        title={cut.title || ""}
        stats={cut.stats}
  ...
  // --- Subscribe CTA scene ---
  if (cut.type === "subscribe_cta") {
    return (
      <SubscribeCTA
        handle={cut.handle || "@AI_Conduit"}
        message={cut.message || "毎日AIトレンドを紹介中"}
        ctaText={cut.ctaText || "コメントにconduitでテンプレ無料プレゼント"}
        accentColor={accent}
        backgroundColor={bgColor || theme.backgroundColor}
      />
    );
