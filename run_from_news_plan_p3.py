...
plan_path = ROOT_DIR / "sns_automation" / "news_content_plan_p3.json"
...
mood_map = {"Hook": "hook", "Why": "why", "Fact_1": "fact_1", "Fact_2": "fact_2", "Impact": "impact", "Twist": "twist", "Context": "context", "CTA": "cta", "Interrupt": "interrupt", "Value": "value"}
scene_specs = []
for s in raw_scenes:
...
MOOD_VISUAL_QUERIES = {
    ...
    "why": f"{topic} impact importance significance",
    "value": f"{topic} technology innovation concept",
    "fact_1": f"{topic} data statistics performance",
    "fact_2": f"{topic} comparison analysis detail",
    "impact": f"{topic} engineer developer working",
    "interrupt": f"{topic} AI speed performance result",
    "twist": f"surprising unexpected revelation concept",
    "context": f"{topic} industry market comparison",
...
# P3: B-roll BはnarrationキーワードでPexels動的検索
def get_broll_b_query(narration, mood):
    """narrationから英語キーワードを抽出してB-roll Bクエリを生成"""
    import re as _re2
    # 数字・%・倍を含む場合はデータ系
    if _re2.search(r"\d+|%|倍|割", narration):
        return "data visualization analytics graph"
    # 速い・高速・向上の場合はスピード系
    if any(w in narration for w in ["速", "高速", "向上", "改善", "強化"]):
        return "fast speed technology performance computer"
    # コスト・価格・安の場合は経済系
    if any(w in narration for w in ["コスト", "価格", "安", "削減", "節約"]):
        return "cost saving money business finance"
    # デフォルト: tech系ランダム
    return _rand_broll.choice(TECH_BROLL_QUERIES)

MOOD_VISUAL_QUERIES_2 = {}  # P3では動的生成
...
for i, spec in enumerate(scene_specs):
    ...
    scene = {
        ...
        "visual_2": get_broll_b_query(sent, spec["mood"]),
        "flash_text": sent if spec["mood"] in ("interrupt", "twist") else "",
    }
    scenes.append(scene)
...
MOOD_TTS_PARAMS = {
    "hook":           {"rate": "+10%", "pitch": "+4Hz"},
    "why":            {"rate": "-5%",  "pitch": "-2Hz"},
    "fact_1":         {"rate": "-3%",  "pitch": "-2Hz"},
    "fact_2":         {"rate": "-3%",  "pitch": "-2Hz"},
    "impact":         {"rate": "+3%",  "pitch": "+2Hz"},
    "twist":          {"rate": "+8%",  "pitch": "+3Hz"},
    "context":        {"rate": "-8%",  "pitch": "-4Hz"},
    "interrupt":      {"rate": "+5%",  "pitch": "+2Hz"},
    "value":          {"rate": "-8%",  "pitch": "-4Hz"},
    "secondary_hook": {"rate": "+5%",  "pitch": "+2Hz"},
    "cta":            {"rate": "+5%",  "pitch": "+6Hz"},
...
final_filename = f"v3news_{safe_topic}.mp4"
final_filename = f"v3news_{safe_topic}.mp4"
