...
WORK_DIR=Path("/tmp/ai_conduit_v19")
...
def generate_timeline(repo,stars,description):
    print("[1/4] 📅 タイムライン生成中...")
    prompt=f"""Write a Japanese timeline video script about the history and evolution related to {repo} ({stars}★) - {description}
Create 8 timeline events from past to present/future.
RULES:
- "year": year or era (e.g. "2010年", "2023年3月", "未来")
- "event": 20-35 chars Japanese event description
- "narration": same as event (spoken)
- "mood": past/turning_point/present/future/cta
- "visual": cinematic Pexels English search term
Output ONLY JSON:
[{{"id":1,"year":"2008年","event":"GitHubが誕生した","narration":"2008年、GitHubが誕生した","mood":"past","visual":"dark history timeline cinematic"}},...]"""
...
def fetch_broll(query):
    ...
    return str(fpath)
MOOD_COLORS={'past':(150,150,150),'turning_point':(255,140,0),'present':(0,200,100),'future':(0,150,255),'cta':(180,80,255),'default':(255,255,255)}
def gen_timeline_overlay(item,out_path,total,idx):
    img=Image.new('RGBA',(1080,1920),(0,0,0,0))
    draw=ImageDraw.Draw(img)
    font_year=get_font(90); font_event=get_font(56); font_logo=get_font(34)
    mood=item.get("mood","past"); color=MOOD_COLORS.get(mood,MOOD_COLORS['default'])
    year=re.sub(r"[\U0001F000-\U0001FAFF⭐]","",item.get("year","")).strip()
    event=re.sub(r"[\U0001F000-\U0001FAFF⭐]","",item.get("event","")).strip()
    # タイムラインライン（縦）
    draw.rectangle([530,100,540,1800],fill=(255,255,255,80))
    # 現在位置ドット
    dot_y=100+int(1700*(idx/max(total-1,1)))
    draw.ellipse([510,dot_y-20,560,dot_y+20],fill=(*color,255))
    # 年号（大きく）
    if year:
        dummy=Image.new('RGBA',(1,1)); dd=ImageDraw.Draw(dummy)
        bb=dd.textbbox((0,0),year,font=font_year)
        yw=bb[2]-bb[0]
        for dx in range(-4,5):
            for dy in range(-4,5):
                if dx*dx+dy*dy<=16: draw.text(((1080-yw)//2+dx,800+dy),year,font=font_year,fill=(0,0,0,200))
        draw.text(((1080-yw)//2,800),year,font=font_year,fill=(*color,255))
    # イベントテキスト
    if event:
        dummy=Image.new('RGBA',(1,1)); dd=ImageDraw.Draw(dummy)
        max_w=960; line=""; lines=[]
        for ch in event:
            test=line+ch; bb=dd.textbbox((0,0),test,font=font_event)
            if bb[2]-bb[0]>max_w and line: lines.append(line); line=ch
            else: line=test
        if line: lines.append(line)
        lh=font_event.size+8; total_h=len(lines)*lh; y=1000
        max_lw=max(dd.textbbox((0,0),l,font=font_event)[2] for l in lines)
        draw.rounded_rectangle([(1080-max_lw)//2-16,y-12,(1080+max_lw)//2+16,y+total_h+12],radius=12,fill=(0,0,0,190))
        for i,l in enumerate(lines):
            bb=dd.textbbox((0,0),l,font=font_event); x=(1080-bb[2])//2
            draw.text((x,y+i*lh),l,font=font_event,fill=(255,255,255,255))
    draw.rectangle([0,1870,1080,1920],fill=(0,0,0,160))
    draw.text((20,1878),"AI Conduit",font=font_logo,fill=(255,255,255,200))
    img.save(out_path,'PNG')
def compose_scene(item,idx,total):
    dur=item["duration"]; audio=item["audio_path"]
    broll=fetch_broll(item.get("visual","dark history timeline cinematic"))
    out=str(WORK_DIR/f"scene_v19_{idx:02d}.mp4")
    if broll and os.path.exists(broll):
        broll_dur=_probe_dur(broll); loop=int(dur/max(broll_dur,1))+2
        bg=str(WORK_DIR/f"bg19_{idx:02d}.mp4")
        _run(["ffmpeg","-y","-stream_loop",str(loop),"-i",broll,"-t",str(dur),
              "-vf","scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,colorchannelmixer=rr=0.35:gg=0.35:bb=0.4",
    ...
    ovr=str(WORK_DIR/f"ovr19_{idx:02d}.png")
    gen_timeline_overlay(item,ovr,total,idx)
    composed=str(WORK_DIR/f"comp19_{idx:02d}.mp4")
...
def main():
    ...
    print(f"\n🚀 AI Conduit Pipeline v19 (Timeline)")
    items=generate_timeline(repo,stars,desc)
    items=gen_narrations(items)
    total=len(items)
    files=[compose_scene(item,i,total) for i,item in enumerate(items)]
    concat=str(WORK_DIR/"concat_v19.txt")
    with open(concat,"w") as f:
        for sf in files: f.write(f"file '{sf}'\n")
    output=str(OUTPUT_DIR/"pipeline_v19_timeline.mp4")
