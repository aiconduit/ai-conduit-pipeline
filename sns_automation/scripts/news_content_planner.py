...
日本のエンジニア向けAIニュースショート動画(15〜20秒・5シーン)のスクリプトを生成してください。

## 最重要ルール
1. narrationは1シーン15〜20文字。短くテンポよく。数字必須。
2. 必ず数字・%・倍を含めること
3. 抽象表現禁止 → 数値で示す
4. visual_descは英語で具体的に

## 選定基準（必ずこの順で評価）
1. 日本のエンジニア・IT学生が「使える」「稼げる」「時短できる」と感じるか
2. 具体的な数字・ツール名・コマンドが含まれるか
3. GPT/OpenAI関連は除外（used_topicsで管理）
4. 速報性よりも「実用性」「独自性」を優先
5. 以下のカテゴリから毎回バランスよく選ぶ:
   - AIツール紹介（Claude/Gemini/Cursor/Copilot等）
   - コーディングテクニック（GitHub/Python/JS等）
   - AI副業・生産性（時短・自動化・収益化）
   - 最新モデル比較・ベンチマーク
   - オープンソースAIツール
...
## シーン構成（合計15〜20秒・5シーン・必ず20秒以内）
【重要】total_duration_secは必ず18以下。長いと再生数激減。ループ視聴を狙う。

1. Hook(3秒): 衝撃数字・事実を0秒から画面表示「〇〇が××に！」
2. Fact(5秒): 最重要事実1つだけ。数字必須
3. Impact(4秒): 視聴者への影響「あなたは〇〇できる」
4. Twist(4秒): 意外な逆転・クリフハンガー（ループ誘発）
5. CTA(3秒): 「コメントにAIと書いてプレゼントゲット！」
