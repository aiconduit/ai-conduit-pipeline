...
タイトル最適化ルール（VidIQ基準）:
- 必ず数字を含む（「3つの方法」「5分で」「月10時間削減」など）
- 検索キーワードを冒頭に配置（ツール名・「AI」「自動化」「無料」など）
- 40文字以内
- 「！」か「？」で終わる
- 「知らない」「無料」「最強」「たった」などの感情ワードを入れる:

```json
[
  {{
    "topic": "YouTube SEO最適化済みタイトル（日本語・40文字以内・数字必須・検索キーワード含む・クリックしたくなるもの。例:『n8nで月10時間削減！無料AI自動化の始め方』）",
    ...
    "hashtags": ["#AI", "#自動化", "#エンジニア", "#プログラミング", "#副業" ...（日本で検索されやすいハッシュタグ5〜8個）],
    "tags": ["ai automation", "ツール名", "free ai tool", "ai 自動化", "エンジニア 副業" ...（YouTube検索で使われる広義・狭義混在の英日タグ10〜15個）],
